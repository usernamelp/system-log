...
public class TreeUtil {
    ...
    public static void main(String[] args) throws IllegalAccessException {
        ...
        menuDTOS.add(root);
        menuDTOS.add(child);
        List<MenuDTO> menuDTOList = buildTreeByRecursion(menuDTOS);
        System.out.println(menuDTOList);
        Function<String, SysUserDTO> convert = MenuDTO::convert;
        for (Field field : root.getClass().getDeclaredFields()) {
            field.setAccessible(true);
            Object o = field.get(root);
            System.out.println(o);
        }
        System.out.println(Arrays.stream(root.getClass().getDeclaredFields()).map(Field::getName).collect(Collectors.toList()));
    }
